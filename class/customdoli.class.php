...
class customdoli extends SeedObject
{
	/**
	 * Draft status
	 */
	...
	public $table_element = 'customdoli';

	public $element = 'customdoli';
...
//		global $user;
	...
	private function getNextNumero()
	{
		...
		$mask = !empty($conf->global->customdoli_REF_MASK) ? $conf->global->customdoli_REF_MASK : 'MM{yy}{mm}-{0000}';
		$numero = get_next_value($db, $mask, 'customdoli', 'ref');
...
//		global $user;
	...
	public function getNomUrl($withpicto=0, $get_params='')
	{
        ...
        $result='';
        $label = '<u>' . $langs->trans("Showcustomdoli") . '</u>';
        if (! empty($this->ref)) $label.= '<br><b>'.$langs->trans('Ref').':</b> '.$this->ref;
        
        $linkclose = '" title="'.dol_escape_htmltag($label, 1).'" class="classfortooltip">';
        $link = '<a href="'.dol_buildpath('/customdoli/card.php', 1).'?id='.$this->getId(). $get_params .$linkclose;
	...
	public static function LibStatut($status, $mode)
	{
		...
		if ($status==self::STATUS_DRAFT) { $statustrans='statut0'; $keytrans='customdoliStatusDraft'; $shortkeytrans='Draft'; }
		if ($status==self::STATUS_VALIDATED) { $statustrans='statut1'; $keytrans='customdoliStatusValidated'; $shortkeytrans='Validate'; }
		if ($status==self::STATUS_REFUSED) { $statustrans='statut5'; $keytrans='customdoliStatusRefused'; $shortkeytrans='Refused'; }
		if ($status==self::STATUS_ACCEPTED) { $statustrans='statut6'; $keytrans='customdoliStatusAccepted'; $shortkeytrans='Accepted'; }
...
/*
class customdoliDet extends TObjetStd
{
	public $table_element = 'customdolidet';

	public $element = 'customdolidet';
