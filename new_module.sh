...
for fic in `find . -iname "*customdoli*" `
do
	# Renommage des variables dans les fichiers
	sed -i 's/customdoli/'$new_module'/g' $fic
	sed -i 's/Customdoli/'$new_module_ucfirst'/g' $fic
	sed -i 's/customdoli/'$new_module_min'/g' $fic
	sed -i 's/100000/'$new_module_number'/g' $fic
	
	# Renommage des fichiers
	OLDNAME=`echo $fic`
	NEWNAME=`echo $fic | sed 's/customdoli/'$new_module'/g'`
	NEWNAME=`echo $NEWNAME | sed 's/customdoli/'$new_module_min'/g'`
	mv $OLDNAME $NEWNAME
done
